...
import {
  ...
  PlusCircle,
  ArrowRight
} from "lucide-react-native"; // Added/updated icons
...
const LandDetailScreen = ({ navigation }) => {
  ...
  const fetchData = useCallback(
    ...
    [landId, refreshing]
  ); // Depend on landId and refreshing
  ...
  const handleNotificationPress = () =>
    Alert.alert("Navigate", "Go to Notifications");
  const handleCropSuggestion = () =>
    navigation.navigate("CropSuggestion", { landId: landId });
  const handleDiagnoseDisease = () => {
    if (!landId) {
         Alert.alert("Error", "Cannot start scan. Land context is missing.");
         return;
    }
    console.log(`Navigating to Scan tab with Land ID: ${landId}, Planting ID: ${currentPlanting?.id}`); // Add Log
    // Navigate to the Scan Tab, passing parameters
    navigation.navigate('AppTabs', { // Navigate to the tab navigator first
        screen: 'Scan', // Specify the target tab screen name
        params: { // Pass params specifically for the 'Scan' screen
            landId: landId,
            // Pass planting ID only if currentPlanting exists and has an id
            plantingId: currentPlanting?.id ?? null
        }
    });
};
  const handleFertiliserRec = () => {
    ...
    if (!currentPlanting) {
      ...
      Alert.alert(
        "Info",
        "Fertilizer recommendations require an active planting."
      );
      return;
    }
  ...
  const handleSoilMonitoring = () => {
    if (!landId) {
       Alert.alert("Error", "Cannot view monitoring. Land ID is missing.");
       return;
   }
   // ++ Navigate to SoilMonitoring with landId ++
   navigation.navigate('SoilMonitoring', { landId: landId });
   // -- Remove Alert --
   // Alert.alert("Navigate", "Go to Soil Monitoring History (WIP)");
};
const handleViewMonitoring = () => handleSoilMonitoring();
  const handleAlertPress = (alert) =>
    Alert.alert("Navigate", `View Alert: ${alert.title} (WIP)`);
  ...
  const handleManagePlanting = () => {
    ...
    Alert.alert(
      "Manage Planting",
      `Manage current planting (ID: ${landDetails?.current_planting?.id}) - WIP`
    );
  };

  // --- Render Logic ---
  // Loading and Error States (keep as before)
  if (isLoading && !landDetails) {
  ...
  if (error && !landDetails) {
    // Show fatal error only if no data loaded
    return (
      <SafeAreaView style={styles.safeArea}>
  ...
  if (!landDetails) {
    // Handle case where landId was invalid or fetch failed quietly after initial load attempt
    return (
      <SafeAreaView style={styles.safeArea}>
  ...
  const areaDisplay = `${landDetails.area || "--"} ${
    landDetails.area_unit || ""
  }`;
  const locationDisplay = `Farm: ${landDetails.farm_name || "Unknown"} - ${
    landDetails.land_name || "Unnamed Plot"
  }`;
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top", "left", "right"]}>
      <CustomHeader
        title={
          userData?.name ? `Hello, ${userData.name.split(" ")[0]}` : "Details"
        }
        showBackButton={true}
      ...
      >
        ...
        <View style={styles.topInfoContainer}>
          ...
          <View style={styles.topRightInfo}>
            <Text
              style={[
                styles.farmTitle,
                !currentPlanting && styles.farmTitleEmpty,
              ]}
            >
              {farmTitle}
            </Text>
        ...
        {!currentPlanting && !isLoading && (
          <View style={styles.emptyPlantingContainer}>
            ...
            <Text style={styles.emptyPlantingText}>
              No active planting on this plot.
            </Text>
            <Text style={styles.emptyPlantingSubText}>
              Select 'Add Planting' below to get started.
            </Text>
          </View>
        )}
            ...
            <>
              ...
              <View style={styles.actionGridRow}>
                <ActionCard
                  ...
                  onPress={handleCropSuggestion}
                  imageUrl={crop_suggestion_bg}
                />
                <ActionCard
                  title="Diagnosing Plant Disease"
                  onPress={handleDiagnoseDisease}
                  imageUrl={disease_bg}
                />
              </View>
              ...
              <View style={styles.actionGridRow}>
                <ActionCard
                  ...
                  onPress={handleFertiliserRec}
                  imageUrl={fertiliser_bg}
                />
                <ActionCard
                  title="Soil Monitoring"
                  onPress={handleSoilMonitoring}
                  imageUrl={monitor_bg}
                />
              </View>
            </>
        ...
        <TouchableOpacity
          style={styles.marketValueButton}
          onPress={() =>
            navigation.navigate("MarketValue", { cropName: landDetails.crop })
          } // Replace 'crop' with the actual property name in your landDetails object that holds the crop name
        >
          <Text style={styles.marketValueButtonText}>
            Get Market Value for Crops <ArrowRight color={"white"} size={14}/>
          </Text>
        </TouchableOpacity>
        ...
        {latestReading && (
          <View style={styles.readingsSection}>
            <SectionHeader
              title="Current Soil reading"
              onViewAll={handleViewMonitoring}
            />
            <View style={styles.npkContainer}>
              <NPKCard
                label="Nitrogen"
                value={latestReading?.nitrogen_value}
                unit="ppm"
              />
              <NPKCard
                label="Phosphorus"
                value={latestReading?.phosphorus_value}
                unit="ppm"
              />
              <NPKCard
                label="Potassium"
                value={latestReading?.potassium_value}
                unit="ppm"
              />
            </View>
            ...
            <View style={[styles.npkContainer, { marginTop: 10 }]}>
              ...
              <NPKCard
                label="Moisture"
                value={latestReading?.moisture_value}
                unit="%"
              />
              <NPKCard
                label="Temp"
                value={latestReading?.temperature_value}
                unit="°C"
              />
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  actionCardSpacer: {
    // Takes space of a card when only one is needed in a row
    flex: 1,
    margin: 6,
  },
